...
@Slf4j
@Service
@Qualifier("TrafiklabClient")
public class TrafiklabClient implements TrafiklabClientInf {
    ...
    @Autowired
    public TrafiklabClient(@Value("${apikey}") final String key,
    ...
    TrafiklabClient(WebClient client) {
        this.client = client;
    }


    private <T> Mono<T> get(Map<String, String> args, ParameterizedTypeReference<T> asResponseType) {
        return client.get()
    ...
    private Mono<ApplicationException> handleHttpError(ClientResponse response) {
        return Mono.just(new InternalErrorException("Bad response from backend: " + response.statusCode()));
    }

    private <U, T extends ResponseData<U>> Mono<List<U>> handleResponseError(Mono<TLResponse<T>> tlResponse) {
        return tlResponse
